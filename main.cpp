...
int main(int argc, char *argv[])
{
    ...
    const AVCodec			*pCodec, *videoCodec;		//解码器
...
//    av_register_all();
    ...
    {
        //3、打开文件
        if(avformat_open_input(&pFormatCtx, /*filename*/argv[1], NULL, NULL) != 0) //error
        {
...
void audio_callback(void *userdata, Uint8 *stream, int len)
{
    ...
    if((NULL != is) && (NULL != stream))  //参数检查
    {
        ...
        while (len > 0) {
            ...
            //音频PTS暂时现这样写，坑了再改
            if(is->audio_st->codecpar)
            {
                //n = 2 * is->audio_st->codec->channels;
                n = is->audioFrame->channels * is->audio_st->codecpar->channels;
                is->audio_clock += (double) len1 / (double) (n * is->audio_st->codecpar->sample_rate);
            }
...
int audio_decode_frame(AVCodecContext *codecCtx, uint8_t *audio_buf, int buf_size) {
    ...
    for (;;) {
        ...
        while (true) {
            ret = avcodec_receive_frame(codecCtx, audioFrame);
